...
public sealed class UnblockConversationCommandHandler(
    ICurrentUserService currentUser,
    IConversationRepository conversationRepository,
    IUnitOfWork unitOfWork)
    : IRequestHandler<UnblockConversationCommand, Result>
{
    public async Task<Result> Handle(UnblockConversationCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!conversation.IsBlocked)
            return Result.Fail(new ConflictError(MessagingMessages.NotBlocked));

        if (conversation.BlockedByUserId != userId)
            return Result.Fail(new ForbiddenError(MessagingMessages.OnlyBlockerCanUnblock));
